...
class ATM:
    ...
    def deposit(self, user, amount):
        user.balance += amount
        user.transaction_history.append("Deposited Rs. " + str(amount))

    def withdraw(self, user, amount):
        if user.balance >= amount:
            user.balance -= amount
            user.transaction_history.append("Withdrew Rs. " + str(amount))
        else:
    ...
    def transfer(self, user, recipient_id, amount):
        if recipient_id in self.users:
            ...
            if user.balance >= amount:
                ...
                user.transaction_history.append("Transferred Rs. " + str(amount) + " to " + recipient_id)
            else:
                print("Insufficient funds")
        else:
